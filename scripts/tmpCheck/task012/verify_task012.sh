...
ck_s06_migrations() {
  local pg="$REPO_ROOT/migrations/versioned/000094_model_call_pricing_snapshot.up.sql"
  local sq="$REPO_ROOT/migrations/sqlite/000017_model_call_pricing_snapshot.up.sql"
  if have "$pg" && have "$sq" && grep -q 'pricing_status' "$pg" && grep -q 'estimated_cost_nanos' "$sq"; then
    record s06.migrations PASS "PG 000094 + SQLite 000017 pricing snapshot migrations present"; return 0
...
run_postgres() {
  ...
  if scripts/tmpCheck/task010/run_postgres_parity.sh "$out" >"$RUN_DIR/postgres.stdout.log" 2>&1 \
      && grep -q '^OK  migrations/versioned/000094_model_call_pricing_snapshot.up.sql$' "$out/migration_postgres.log" \
      && [ ! -s "$out/parity.diff" ]; then
    record d02.postgres_migration PASS "ephemeral PostgreSQL applied 000094; report parity equals SQLite"
    return 0
  fi
