...
func TestSQLiteMigrationsModelCallsRepeatAndDown(t *testing.T) {
	...
	require.Equal(t, uint(9), latest, "SQLite migration set must include the pricing snapshot migration")

	// Repeat up is a no-op, not an error.
	require.ErrorIs(t, m.Up(), migrate.ErrNoChange)

	// One step down drops the pricing snapshot migration (000009). The
	...
	require.False(t, sqliteColumnExists(t, dbPath, "model_calls", "pricing_status"), "down must remove the pricing snapshot columns")
	require.False(t, sqliteColumnExists(t, dbPath, "model_calls", "estimated_cost_nanos"), "down must remove estimated_cost_nanos")
	require.True(t, sqliteColumnExists(t, dbPath, "model_calls", "cache_reported_input_tokens"), "down must not remove earlier additive column")
	...
	require.True(t, sqliteColumnExists(t, dbPath, "model_calls", "pricing_status"), "up must restore pricing snapshot columns")
	require.True(t, sqliteColumnExists(t, dbPath, "model_calls", "estimated_cost_nanos"), "up must restore estimated_cost_nanos")
}
