...
func TestSQLiteMigrationsEmbeddingCacheRepeatAndDown(t *testing.T) {
	...
	require.Equal(t, uint(9), latest, "SQLite migration set must include the embedding cache migration")

	require.ErrorIs(t, m.Up(), migrate.ErrNoChange)

	// Two steps down (pricing snapshot 000009, then embedding cache 000008)
