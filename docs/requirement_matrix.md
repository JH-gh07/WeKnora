...
| R9 | 干净环境复现 | 避免依赖本机隐式状态 | Reproduction command | fresh clone | reproduction log | P0-B | 独立复现者 | PLAN_READY / EXECUTION_BLOCKED_BY_INDEPENDENT_REPRODUCER_AND_FREEZE（G5/G6 已 GO） | tests/evaluation + task009 contract | Official Core 必须 deterministic/offline/no-secret；Provider Experiment 仅 advisory。尚需 remote full SHA/tree、RC 预登记、独立执行者与 disposable environment |
