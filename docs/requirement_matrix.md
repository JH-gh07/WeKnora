...
| R2 | 结果持久化 | Run 生命周期如何保存 | EvaluationRun DB | kill/restart | DB evidence | P0-A | 本人 | VERIFIED（实现范围内） | internal/types/evaluation_run.go + internal/application/repository/evaluation_run.go + migrations/versioned/000085 + migrations/sqlite/000005 | 中断 run 无精确 partial aggregate（首版不承诺） |
| R3 | 四类结果（质量/成本/耗时/用量） | 数据如何统一关联 | Metric/Usage | Run 查询 | report | P0-A | 本人 | CODED（Task010「统一关联/呈现」已实现：run 级统一报告 API + Run Detail 页；SQLite/PG parity/live-HTTP/RBAC/failure/性能/负控 N01–N08 证据已闭合；AC-R3 GO — jhhh 签字 rc2 `00384be7`，browser B01–B18 18/18 PASS） | internal/application/service/evaluation_report.go + internal/types/evaluation_report.go + internal/handler/evaluation.go + internal/router/routes_infra.go + frontend/src/views/evaluation/* + scripts/tmpCheck/task010/* | 真实 Provider 费用覆盖仍存在缺口（未知价格保持 UNKNOWN，不写 0）；run 级计量完整性仍 UNKNOWN |
| R4 | CI 门禁 | 如何判断 regression | RegressionPolicy + Comparator | 故意降 recall | failed CI | P0-A | 本人 | VERIFIED_WITH_BOUNDARY（G5 GO；local deterministic core=`VERIFIED_LOCAL`；Owner 授权 `JH-gh07/WeKnora` fork governance=`VERIFIED_EXTERNAL`：Ruleset `21725115` active/strict，精确 context `evaluation-regression / quality`；健康 PR #2 PASS 并合并，负控 PR #3 COMPARABLE→BLOCK/exit 2、merge box BLOCKED 且未合并；「定时执行」Task011/AC-R4 进行中：scheduled workflow `evaluation-regression-scheduled / quality` 已写入（cron `17 2 * * 1-5` + no-input dispatch，contents:read，C01–C14/S01–S15 + 负控 N01–N10 + 本地 L01–L10 全 PASS），真实 `event=schedule` 运行证据待外部部署后观察 → AC-R4 IN_PROGRESS/BLOCKED_EXTERNAL_CONFIGURATION） | internal/application/service/evaluation_regression*.go + cmd/evaluation-regression + tests/evaluation/* + .github/workflows/evaluation-regression.yml + scripts/tmpCheck/task007/* | Owner 通过 `CR-2026-08-30-001` 撤销 09-04 内部日期门禁，当前反馈无 G5 MUST 冲突；fork 证据不外推为 Tencent 上游 Ruleset 已配置。PASS run `33163214435`，BLOCK run `33163682389`；最终认证 audit exit 0 |
| R5 | 调用与成本统计 | 如何统一采集并保持计价可解释 | ModelCall + HistoricalPricingIdentity | 数据库核对 | SQL/API | P0-A | 本人 | VERIFIED_WITH_BOUNDARY（G2A：unknown-cost/DB 已验证；G2B 已验证 currency-safe aggregate，混币/缺币 fail closed）；Task012（one-provider 真实 usage-based 计价）进行中：SiliconFlow Qwen/Qwen3-14B catalog+estimator+recorder+双库迁移离线全绿，live non-stream/stream 4/4 PRICED 且独立重算 MATCH，端到端（PG/完整 API/UI/Evaluation Run/browser）待验收环境 → AC-R5 未标 GO | internal/models/*（含 internal/models/pricing） | SDK 隐藏 retry；除 SiliconFlow Qwen/Qwen3-14B 外其余 Provider/模型仍 UNKNOWN；Task012 未完成端到端前不宣称 R5 one-provider 闭合 |
...
| R9 | 干净环境复现 | 避免依赖本机隐式状态 | Reproduction command | fresh clone | reproduction log | P0-B | 独立复现者 | VERIFIED / G7 GO（jhhh textual signoff；RC 0.8.0-rc1 @ 20a7f837；Bootstrap claim、author E1/PG/full verifier PASS） | tests/evaluation + task009 contract | 当前签字不覆盖 Task010 后的新 SHA；Release publish 仍需单独授权 |
...
- [x] `EvaluationRun` 最小持久化闭环：PostgreSQL(000085) + SQLite(000005) 两套 migration，租户隔离 repository（create/find/update/list）。
...
- [x] PostgreSQL 实机 migration/restart 验证：000080-000085 实机 up（ON_ERROR_STOP=1）+ 重复 up + down 均通过（ParadeDB pg17）；live HTTP T1/T2（含 `docker compose kill app` 中断重启 reconciliation）PASS=7 FAIL=0；SQLite migration + restart + tenant 验证已先行完成。详见 `status/evidence/task002/migration_postgres.log`、`t2_interrupted_run.md`。
...
- [x] 双库 migration：PostgreSQL `000086` + SQLite `000006`（model_calls + model_metering_health），tenant/time/run/model 索引。
...
- [x] live 验证完成：PG 实机 migration（85→86）+ scratch 库 repeat/down/up 矩阵 + 重启聚合一致（restart_query.md）；受控采样发现并修复 purpose 缺口（8 处 setter，重采样验证）；租户隔离双向验证；OFF/ON overhead（persist median 0.82ms/p95 1.63ms，全路径 ORM 上界 ~2.5–2.7ms，同步 durable write 维持，不声明严格 <0.1%）；失败注入（业务 200 + PARTIAL 降级 + 恢复）。见 task003_review.md。
