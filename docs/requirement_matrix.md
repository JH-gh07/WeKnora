...
| R4 | CI 门禁 | 如何判断 regression | RegressionPolicy + Comparator | 故意降 recall | failed CI | P0-A | 本人 | NOT_STARTED | .github/workflows | 外部 provider 波动 |
| R5 | 调用与成本统计 | 如何统一采集并保持计价可解释 | ModelCall + HistoricalPricingIdentity | 数据库核对 | SQL/API | P0-A | 本人 | VERIFIED（G2A） | internal/models/* | SDK 隐藏 retry |
| R6 | 模型管理页 | 如何按模型和时间聚合 + 租户隔离 | Usage API + ModelSettings UI | 筛选与权限测试 | UI/demo | P0-B | 本人 | IN_PROGRESS（G2A Usage API 已 VERIFIED；G2B UI/RBAC/E2E 尚未开始） | frontend/src/views/settings | 官方验收项；仅消费 G2A 稳定 API/Availability，不重定义计量 grain |
| R7 | Embedding Cache | Key/失效/并发/租户隔离 | EmbeddingCache | cold/warm/model-change | before/after | P0-B | 本人 | NOT_STARTED | internal/models/embedding | 缓存键身份；不扩展 TTL/LRU/GC/通用 Cache 平台 |
| R8 | Prompt Cache | 现有实现是否提高 cached-token ratio | 现状审计 + A/B Harness | 同文档同模型实验 | reporting coverage + ratio | P0-B | 本人 | NOT_STARTED | internal/models/chat | provider 不可报告时允许诚实 INCONCLUSIVE，不建设新 Prompt Cache 平台 |
